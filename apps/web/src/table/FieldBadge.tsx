...
export function FieldBadge({ field, isForeignKey, isPk }: { field: Field; isForeignKey: boolean; isPk: boolean }) {
  if (isPk) return <KeyIcon size={16} className="table-node-row-icon table-node-row-icon-pk" />;
  if (field.unique) return <DiamondIcon size={16} className="table-node-row-icon table-node-row-icon-unique" />;
  if (isForeignKey) return <LinkIcon size={16} className="table-node-row-icon" />;
