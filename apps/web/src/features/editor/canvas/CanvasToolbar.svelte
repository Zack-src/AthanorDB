...
<script lang="ts">
  ...
  import ViewModeToggle, { type EditorViewMode } from "@/features/editor/mcd/ViewModeToggle.svelte";
  import DetailLevelDropdown from "./DetailLevelDropdown.svelte";
  import InsertToolDropdown from "./InsertToolDropdown.svelte";
  ...
  // The reset-link-routing command ships with the app — it used to only be
  // reachable through the plugin dropdown, which read as "this is some
  // third-party plugin's action" for a core editing command. Surfaced here as
  // its own button; `PluginMenu` keeps only what real plugins contribute.
  const resetLinkRoutingCommand = $derived(
    props.canvasCommands.find(
...
<AnimatedToolbarPill pillId="editing-toolbar">
  ...
  <span class={CANVAS_TOOLBAR_DIVIDER_CLASS}></span>
  <!-- `data-testid`: a locale-independent handle for the perf harness
       (scripts/bench-web.mjs), which drives this exact button — every other
       selector here is a translated string. -->
  <button
    type="button"
    class={`${CANVAS_TOOLBAR_ICON_BTN_CLASS} ${props.highlightLinks ? CANVAS_TOOLBAR_TOGGLE_ACTIVE_CLASS : ""}`}
  ...
  >
    <Icon icon={LinkIcon} size={TOGGLE_ICON_SIZE} />
  </button>
  <button
    type="button"
    class={`${CANVAS_TOOLBAR_ICON_BTN_CLASS} ${props.showValidationIssues ? CANVAS_TOOLBAR_TOGGLE_ACTIVE_CLASS : ""}`}
  ...
  >
    <Icon icon={AlertTriangleIcon} size={TOGGLE_ICON_SIZE} />
  </button>
  {#if props.canWrite && resetLinkRoutingCommand}
    <button
      type="button"
    ...
    >
      <Icon icon={RestoreIcon} size={TOGGLE_ICON_SIZE} />
    </button>
  {/if}
  ...
  <button
    type="button"
    class={`${CANVAS_TOOLBAR_ICON_BTN_CLASS} ${props.minimapVisible ? CANVAS_TOOLBAR_TOGGLE_ACTIVE_CLASS : ""}`}
  ...
  >
    <Icon icon={MinimapIcon} size={TOGGLE_ICON_SIZE} />
  </button>
  <button
    type="button"
    class={`${CANVAS_TOOLBAR_ICON_BTN_CLASS} ${props.searchOpen ? CANVAS_TOOLBAR_TOGGLE_ACTIVE_CLASS : ""}`}
  ...
  >
    <Icon icon={SearchIcon} size={TOGGLE_ICON_SIZE} />
  </button>
