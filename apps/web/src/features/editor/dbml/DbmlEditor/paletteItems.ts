...
import type { TranslationKey, TranslateOptions } from "@/i18n/translate";
import type { PaletteItem, PluginEditorCommand } from "./types";

type Translate = (key: TranslationKey, options?: TranslateOptions) => string;

/**
 * Builds the command palette's entries, in both its "symbols" and "commands"
 * modes.
 *
 * A builder rather than a derived list: the symbol list is read out of the
 * live CodeMirror state, which is not reactive state at all. Calling this the
 * moment the palette opens — from the event that opens it — reads the document
 * as it is right then, by construction.
 */
export function buildPaletteItems(
  view: EditorView,
  palette: "symbols" | "commands",
  options: {
