    ...
    >
      <div className="flex max-h-[70%] w-[92%] flex-col overflow-hidden rounded-lg border border-border bg-surface shadow-2xl">
        <input
          ...
          className="w-full border-b border-border bg-transparent px-3 py-2.5 text-[13px] text-text outline-none placeholder:text-text-muted"
        />
        <div ref={listRef} className="min-h-0 flex-1 overflow-y-auto py-1">
          {filtered.length === 0 && <div className="px-3 py-2 text-[12px] text-text-muted">No match</div>}
          {filtered.map((item, i) => (
            <button
              key={item.id}
              data-idx={i}
