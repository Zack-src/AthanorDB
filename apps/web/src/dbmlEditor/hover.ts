...
import { getSymbols, tableAt, unquoteIdent, type EnumSymbol, type TableSymbol } from "./symbols.js";
import { tokenAt } from "./navigation.js";
...
function tableCard(table: TableSymbol, highlightField?: string): HTMLElement {
  ...
  title.append(el("span", "cm-dbml-hover-kind", "table"), document.createTextNode(" "), el("span", "", table.schema ? `${table.schema}.${table.name}` : table.name));
  ...
  for (const field of table.fields.slice(0, 24)) {
    const row = el("div", `cm-dbml-hover-row${highlightField && field.name.toLowerCase() === highlightField.toLowerCase() ? " is-current" : ""}`);
    row.append(el("span", "cm-dbml-hover-name", field.name));
    row.append(el("span", "cm-dbml-hover-type", field.type));
    const flags = [field.pk && "pk", field.unique && "unique", field.notNull && "not null", field.increment && "increment"]
      .filter(Boolean)
      .join(", ");
...
export const dbmlHover = hoverTooltip((view, pos) => {
  ...
  if (owner && field) {
    const root = el("div", "cm-dbml-hover");
    const title = el("div", "cm-dbml-hover-title");
    title.append(el("span", "cm-dbml-hover-kind", "column"), document.createTextNode(" "), el("span", "", `${owner.name}.${field.name}`));
