...
**Revérifié le 2026-08-21** contre le code réel (pas seulement contre `todo.md`) : deux affirmations de ce document s'étaient périmées sans être corrigées — le thème clair (§4, marqué manquant) et l'avis `esbuild`/`vite` (§1, marqué restant) sont tous deux résolus depuis. Ce document reste un **audit daté**, pas un tracker vivant — pour l'état courant ligne à ligne, `docs/todo.md` est la source à jour ; celui-ci garde son intérêt pour la vue produit/stratégique et l'historique du raisonnement.
...
- ✅ **Dépendances vulnérables** — `npm audit` remontait 7 vulnérabilités (2 modérées, 5 hautes) au 2026-08-09 : `fast-uri` (host confusion via backslash — haute), `js-yaml` (DoS quadratique — haute), `nanoid` (générateur qui boucle à l'infini — haute) ont été corrigées dans la foulée de cet audit (`npm audit fix`, non-breaking, build/lint/125 tests revérifiés après). La chaîne `esbuild`/`vite` restante a été fermée le 2026-08-14 par le bump vers Vite 8 (rolldown-vite, `ee496aa`) — ce même commit a aussi monté `@dbml/core` de 3.x à 10.1.0. **Revérifié le 2026-08-21** : `npm audit` rapporte **0 vulnérabilité**.
...
- ✅ **Révocation de sessions** — fait le 2026-08-09 : `GET /api/auth/sessions` liste vos propres sessions (appareil, IP, dernière activité, session courante repérée), avec révocation unitaire et « déconnecter les autres appareils ». Interface dans _Paramètres → Profil_. Vérifié en navigateur : deux sessions listées, la session curl révoquée depuis l'UI, la session courante conservée.
- ✅ **2FA/MFA (TOTP)** — fait le 2026-08-15 : authentification à deux facteurs par application (TOTP/HOTP, RFC 6238/4226, implémenté sans dépendance et vérifié contre les vecteurs de test officiels de la RFC 4226), codes de secours à usage unique, connexion en deux étapes. Détail complet dans `todo.md` Phase 19. Reste hors périmètre : les passkeys (**L**).
...
- ✅ **Plafond de projets par compte** — fait le 2026-08-09 : 500 projets possédés par compte (409 au-delà). Garde-fou anti-abus, pas une limite commerciale ; les limites d'entités _dans_ un projet existaient déjà (Phase 13).
...
- ✅ **Thème clair** — fait entre le 2026-08-09 et le 2026-08-15 (voir `todo.md` Phase 22) : le sélecteur de thème propose désormais un vrai thème clair, en plus du sombre par défaut (Obsidian). Deux autres presets sombres (Midnight, Emerald) restent dans le sélecteur mais désactivés — visibles, honnêtement marqués « bientôt disponible », sans CSS derrière pour l'instant.
...
- 🟠 **Bundle principal encore volumineux** — mesuré à nouveau le 2026-08-21 : 528,75 Ko brut / 157,26 Ko gzip pour le chunk `index` (mieux que les 707/222 Ko de l'audit initial — React Flow et CodeMirror sont maintenant dans leurs propres chunks, `xyflow` 236 Ko et `codemirror` 397 Ko, `jspdf` 399 Ko toujours lazy). Vite avertit toujours sur ce chunk. **M** — encore de la marge sur `manualChunks` avant de remonter le seuil d'alerte.
...
**Phase D — Application & rollback (L-XL)** — ✅ **essentiellement fait le 2026-08-15**, détail complet dans `todo.md` Phase 27 :

- ~~Exécuter la migration générée contre la base cible, avec transaction quand le dialecte le permet, et un chemin de rollback~~ — fait : migration inverse générée à l'application (`rollbackGenerator.ts`), stockée et rejouable depuis l'historique. Honnêtement qualifiée de "meilleur effort" — pas une restauration depuis un instantané : les données réellement supprimées ne peuvent pas revenir, et c'est signalé explicitement plutôt que promis.
- ~~Historique des migrations appliquées, séparé par environnement~~ — fait : chaque connexion porte désormais une étiquette d'environnement libre (production, staging, …), et chaque déploiement (et chaque rollback) est enregistré avec cette étiquette, qui/quand/combien d'instructions. Nuance : c'est un historique par connexion avec une étiquette d'affichage, pas un vrai concept de pipeline dev→staging→prod que l'app comprendrait et orchestrerait — voir `todo.md` pour la distinction exacte.
- Trouvé et corrigé en cours de route : le pilote MySQL rapportait toujours `0` instruction exécutée en cas d'échec, quel que soit ce qui avait réellement été appliqué avant l'échec (MySQL ne supporte pas le DDL transactionnel) — corrigé en exécutant instruction par instruction, avec un compte exact et vérifié en conditions réelles contre un conteneur MySQL.
...
Ajouté au complément du 2026-08-09. La documentation _opérateur_ est bonne (README : installation, config, Docker, backup/restore, premier admin, comptes et invitations). Ce qui manque est la documentation _utilisateur_ et tout ce qui entoure une version publiée.
...
- ✅ **Le palier « Cloud Pro » à 12 €/utilisateur/mois et son « essai gratuit »** (dont le bouton ouvrait en réalité l'application auto-hébergée) sont remplacés par une carte « Cloud géré — En préparation » : pas de prix, pas de bouton, et une phrase explicite — _« Pas encore ouvert : aucune inscription, aucun essai, aucun paiement. »_ L'introduction de la section annonce désormais sa propre convention : une coche = existe aujourd'hui, une horloge = prévu. C'était le vrai problème — une liste où les deux se ressemblaient.
...
| Décision écrite : mobile, langue (§4, §5)                    | ⬜ décision à prendre                            |
| Thème clair (§4)                                              | ✅ fait — plus une décision à trancher            |

**Hors périmètre V1, assumé** (à écrire noir sur blanc plutôt qu'à laisser croire) : SSO, passkeys (le 2FA par application TOTP est fait, voir §1), API publique, webhooks, mode hors-ligne réel, l'automatisation CI/CD de la §7 (Phase E — le reste de la §7 est fait, voir `todo.md` Phase 27), marketplace de plugins.
...
4. **Décisions à trancher, pas à repousser** (chacune coûte une phrase si elle est assumée, un chantier si elle est niée) : desktop-only ou responsive, français ou i18n réelle, licence MIT face à une offre hébergée, existence ou non d'un produit cloud. (Le thème clair a été construit depuis — retiré de cette liste.)
5. **Moyen terme** : API publique + clés scopées (débloque webhooks et la Phase E de la §7 — CI/CD), templates de projet, comparaison inter-projets, harmonisation visuelle landing/app.
6. **Le grand chantier** : §7 (lien DB + déploiement), en 5 phases indépendantes, chacune avec sa propre revue de sécurité _avant_ implémentation — c'est ce qui différencierait le plus AthanorDB de la concurrence (dbdiagram.io/DrawSQL n'ont pas ça), mais c'est aussi, de loin, le plus risqué si bâclé.
